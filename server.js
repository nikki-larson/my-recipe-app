...
app.get('/api/weekly-recipes', (req, res) => {
  const weeklySelection = getWeeklyRecipes();
...
app.post('/api/grocery-list', express.json(), (req, res) => {
  // Expecting 'recipes' and 'adHocItems' in the request body
  const { recipes, adHocItems } = req.body;

  if (!recipes || !Array.isArray(recipes)) {
      console.log('Validation failed: Recipes is invalid or missing.'); // NEW DEBUG LOG
  ...
  if (adHocItems && Array.isArray(adHocItems)) {
      adHocItems.forEach(adHocItem => {
          if (adHocItem.name) { // Ensure item has a name
              const normalizedItem = adHocItem.name.trim().toLowerCase();
              groceryListMap.set(normalizedItem, (groceryListMap.get(normalizedItem) || 0) + 1);
              // Store the ad-hoc item's preferred category IF it's not already known
              // This gives priority to the user's explicit choice for ad-hoc items
  ...
  Array.from(groceryListMap.entries())
    ...
    .forEach(([ingredient, count]) => {
        ...
        const formattedItem = count > 1 ? `${ingredient} (x${count})` : ingredient;
